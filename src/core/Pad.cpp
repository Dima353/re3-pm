...
#define _CHEATCMP(str)	strncmp(str, CheatString, sizeof(str)-1)
	// "3414LDRULDRU" - R1 R2 L1 R2 LEFT DOWN RIGHT UP LEFT DOWN RIGHT UP
	if ( !_CHEATCMP("URDLURDL4143") )
		WeaponCheat1();

	// "3414LDRULDDL" - R1 R2 L1 R2 LEFT DOWN RIGHT UP LEFT DOWN DOWN LEFT
	else if ( !_CHEATCMP("LDDLURDL4143") ) 
		WeaponCheat2();

	// "3414LDRULDDD" - R1 R2 L1 R2 LEFT DOWN RIGHT UP LEFT DOWN DOWN DOWN
	else if ( !_CHEATCMP("DDDLURDL4143") )
		WeaponCheat3();

	// "341XLDRULDRU" - R1 R2 L1 X LEFT DOWN RIGHT UP LEFT DOWN RIGHT UP
	else if ( !_CHEATCMP("URDLURDLX143") )
		ArmourCheat();

	// "341CLDRULDRU" - R1 R2 L1 CIRCLE LEFT DOWN RIGHT UP LEFT DOWN RIGHT UP
	else if ( !_CHEATCMP("URDLURDLC143") )
		HealthCheat();

	// "33C4LRLRLR" - R1 R1 CIRCLE R2 LEFT RIGHT LEFT RIGHT LEFT RIGHT
	else if ( !_CHEATCMP("RLRLRL4C33") )
		WantedLevelUpCheat();

	// "33CR2UDUDUD" - R1 R1 CIRCLE R2 UP DOWN UP DOWN UP DOWN
	else if ( !_CHEATCMP("DUDUDU4C33") )
		WantedLevelDownCheat();

	// "4X11222D - R2 X L1 L1 L2 L2 L2 DOWN
	...
	// "4X11222X" - R2 X L1 L1 L2 L2 L2 X
	else if ( !_CHEATCMP("X22211X4") )
		FoggyWeatherCheat();

	// "CC1CCC123TCT" - CIRCLE CIRCLE L1 CIRCLE CIRCLE CIRCLE L1 L2 R1 TRIANGLE CIRCLE TRIANGLE
	else if ( !_CHEATCMP("TCT321CCC1CC") )
		VehicleCheat(MI_RHINO);

	// "CC1S1SSS1TCT" - CIRCLE CIRCLE L1 SQUARE L1 SQUARE SQUARE SQUARE L1 TRIANGLE CIRCLE TRIANGLE
	else if ( !_CHEATCMP("TCT1SSS1S1CC") )
		FastWeatherCheat();

	// "423124STCT21" - R2 L2 R1 L1 L2 R2 SQUARE TRIANGLE CIRCLE TRIANGLE L2 L1
	else if ( !_CHEATCMP("12TCTS421324") )
		BlowUpCarsCheat();

	// "RRLU12LUDR" - RIGHT RIGHT LEFT UP L1 L2 LEFT UP DOWN RIGHT
	else if ( !_CHEATCMP("RDUL21ULRR") )
		ChangePlayerCheat();

	// "DLULX4321" - DOWN LEFT UP LEFT X R2 R1 L2 L1
	else if ( !_CHEATCMP("1234XLULD") )
		MayhemCheat();

	// "DUUUX4322" - DOWN UP UP UP X R2 R1 L2 L2
	else if ( !_CHEATCMP("2234XUUUD") )
		EverybodyAttacksPlayerCheat();

	// "43XTXTUD" - R2 R1 X TRIANGLE X TRIANGLE UP DOWN
	else if ( !_CHEATCMP("DUTXTX34") )
		WeaponsForAllCheat();

	// "T33L3141" - TRIANGLE R1 R1 LEFT R1 L1 R2 L1
	else if ( !_CHEATCMP("1413L33T") )
		FastTimeCheat();

	// "TURDS43" - TRIANGLE UP RIGHT DOWN SQUARE R2 R1
	else if ( !_CHEATCMP("34SDRUT") )
		SlowTimeCheat();

	// "T1T4S11" - TRIANGLE L1 TRIANGLE R2 SQUARE L1 L1
	...
	// "4C32L3142" - R2 CIRCLE R1 L2 LEFT R1 L1 R2 L2
	else if ( !_CHEATCMP("2413L23C4") )
		StrongGripCheat();

	// "4CU1R3RUST" - R2 CIRCLE UP L1 RIGHT R1 RIGHT UP SQUARE TRIANGLE
	else if ( !_CHEATCMP("TSUR3R1UC4") )
		DoShowChaseStatCheat();

	// "R1C2LX311X" - RIGHT L1 CIRCLE L2 LEFT X R1 L1 L1 X
	...
	// "R3U22L3133" - RIGHT R1 UP L2 L2 LEFT R1 L1 R1 R1
	else if ( !_CHEATCMP("3313L22U3R") )
		TrafficLightsCheat();

	// "C1D2LX31RC" - CIRCLE L1 DOWN L2 LEFT X R1 L1 RIGHT CIRCLE
	else if ( !_CHEATCMP("CR13XL2D1C") )
		PinkCarsCheat();

	// "C2U3LX31LC" - CIRCLE L2 UP R1 LEFT X R1 L1 LEFT CIRCLE
	else if ( !_CHEATCMP("CL13XL3U2C") )
		BlackCarsCheat();

	// "D3C22X31LL" - DOWN R1 CIRCLE L2 L2 X R1 L1 LEFT LEFT
	else if ( !_CHEATCMP("LL13X22C3D") )
		VehicleCheat(MI_BLOODRA);

	// "D4D32L31LR" - DOWN R2 DOWN R1 L2 LEFT R1 L1 LEFT RIGHT
	else if ( !_CHEATCMP("RL13L23D4D") )
		VehicleCheat(MI_ROMERO);

	// "4U2LL31CR" - R2 UP L2 LEFT LEFT R1 L1 CIRCLE RIGHT
	else if ( !_CHEATCMP("RC13LL2U4") )
		VehicleCheat(MI_LOVEFIST);

	// "C3C3LL31CR" - CIRCLE R1 CIRCLE R1 LEFT LEFT R1 L1 CIRCLE RIGHT
	else if ( !_CHEATCMP("RC13LL3C3C") )
		VehicleCheat(MI_TRASH);

	// "URR1RUS2" - UP RIGHT RIGHT L1 RIGHT UP SQUARE L2
	else if ( !_CHEATCMP("2SUR1RRU") )
		VehicleCheat(MI_BLOODRB);

	// "R2D22X31CL" - RIGHT L2 DOWN L2 L2 X R1 L1 CIRCLE LEFT
	else if ( !_CHEATCMP("LC13X22D2R") )
		VehicleCheat(MI_SABRETUR);

	// "C1U32X31CX" - CIRCLE L1 UP R1 L2 X R1 L1 CIRCLE X
	else if ( !_CHEATCMP("XC13X23U1C") )
		VehicleCheat(MI_CADDY);

	// "3C4R12XXS3" - R1 CIRCLE R2 RIGHT L1 L2 X X SQUARE R1
	else if ( !_CHEATCMP("3SXX21R4C3") )
		VehicleCheat(MI_HOTRINA);

	// "41CR13RUC4" - R2 L1 CIRCLE RIGHT L1 R1 RIGHT UP CIRCLE R2
	else if ( !_CHEATCMP("4CUR31RC14") )
		VehicleCheat(MI_HOTRINB);

	// "C2LX31X1" - CIRCLE L2 LEFT X R1 L1 X L1
	else if ( !_CHEATCMP("1X13XL2C") )
		ChangePlayerModel("igbuddy");

	// "C4D3LR31X2" - CIRCLE R2 DOWN R1 LEFT RIGHT R1 L1 X L2
	else if ( !_CHEATCMP("2X13RL3D4C") )
		ChangePlayerModel("igcandy");

	// "R1U21R31X3" - RIGHT L1 UP L2 L1 RIGHT R1 L1 X R1
	else if ( !_CHEATCMP("3X13R12U1R") )
		ChangePlayerModel("igken");

	// "3C41R31X4" - R1 CIRCLE R2 L1 RIGHT R1 L1 X R2
	else if ( !_CHEATCMP("4X13R14C3") )
		ChangePlayerModel("ighlary");

	// "D1D2LX31XX" - DOWN L1 DOWN L2 LEFT X R1 L1 X X
	else if ( !_CHEATCMP("XX13XL2D1D") )
		ChangePlayerModel("igjezz");

	// "R3U41R31RC" - RIGHT R1 UP R2 L1 RIGHT R1 L1 RIGHT CIRCLE
	else if ( !_CHEATCMP("CR13R14U3R") )
		ChangePlayerModel("igphil");

	// "C1C2LX31XX" - CIRCLE L1 CIRCLE L2 LEFT X R1 L1 X X
	else if ( !_CHEATCMP("XX13XL2C1C") )
		ChangePlayerModel("igsonny");

	// "41U1R3RUCT" - R2 L1 UP L1 RIGHT R1 RIGHT UP CIRCLE TRIANGLE
	else if ( !_CHEATCMP("TCUR3R1U14") )
		ChangePlayerModel("igmerc");

	// "1234D142" - L1 L2 R1 R2 DOWN L1 R2 L2
	else if ( !_CHEATCMP("241D4321") )
		ChangePlayerModel("igdiaz");

	// "R4C32S34" - RIGHT R2 CIRCLE R1 L2 SQUARE R1 R2
	else if ( !_CHEATCMP("43S23C4R") )
		BackToTheFuture();

	// "3XTR4SUDS" - R1 X TRIANGLE RIGHT R2 SQUARE UP DOWN SQUARE
	else if ( !_CHEATCMP("SDUS4RTX3") )
		SpecialCarCheats();

	// "CX114XXCT" - CIRCLE X L1 L1 R2 X X CIRCLE TRIANGLE
